...
def main(path_to_videos='', path_to_labels='', base_path_to_chk_pts=''):
	...
	video_data = video_data[0:50]
	rgb_input = tf.placeholder(tf.float32, shape=(None, 21, _IMAGE_SIZE, _IMAGE_SIZE, 3))
	...
	flow_input = tf.placeholder(tf.float32, shape=(None, 21, _IMAGE_SIZE, _IMAGE_SIZE, 2))
	...
	with tf.Session() as sess:
		...
		for idx, vid in enumerate(video_data):
			final_pred = np.zeros(400)
			print(vid['video'])
			try:
				loader.initializeLazy(vid['video'], 5, 21)
				while(loader.hasNextBatch()):
					rgb = loader.nextBatchFrames()
					flow = loader.nextBatchFlows()
					...
					final_pred += np.sum(out_predictions, axis=0)
